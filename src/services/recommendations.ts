...
export async function fetchUserRecommendations(userId: string, startPage: number) {
    const limit = 20;
    ...
    while (tmdbResults.length < limit && currentPage < maxPages) {
        ...
        const filtered = movies.results.filter((movie: Movie) => !watchlistIdSet.has(movie.id));

...
describe("Reactions Routes", () => {
  ...
  describe("POST /reactions", () => {
    ...
    it("creates multiple reactions in bulk", async () => {
      ...
      expect(response.body.reactions).toEqual(
        expect.arrayContaining([
          ...
          expect.objectContaining({ movieId: secondMovieId, reaction: "reject" }),
        ]),
      );
    });
    ...
    it("upserts when a reaction already exists for the same user and movie", async () => {
      ...
      expect(response.status).toBe(201);
      expect(response.body.reactions[0]).toMatchObject({ movieId, reaction: "reject" });

      const count = await prisma.movieReaction.count({ where: { userId, movieId } });
      expect(count).toBe(1);
    });
